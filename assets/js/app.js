...
(function initPricing() {
  ...
  window.DATA.pricingGroups.forEach((g, idx) => {
    ...
    g.plans.forEach((p) => {
      ...
      card.innerHTML = `
        <h3>${p.label}</h3>
        <p class="price"><span class="currency">A$</span>${Number(
          p.price
        ).toLocaleString()}</p>
        <ul class="features">${ul}</ul>
        <button class="btn select-plan" data-plan="${g.id}-${
        p.id
      }" data-name="${g.label} – ${p.label}" data-price="${
        p.price
      }">Select</button>
      `;
...
(function payment() {
  ...
  const plan = JSON.parse(localStorage.getItem("selectedPlan") || "null");
  if (mini && plan) {
    mini.textContent = `${plan.name} — A$ ${plan.price.toLocaleString()}`;
  }
